...
# 读取数据
data = pd.read_csv('./data/zhizaoye.csv' , encoding="utf-8",engine = "python")
data2 = pd.read_csv('./data/zhizaoye.csv',encoding="utf-8",engine = "python")
print("data：\n")
print(data)
...
#smo = SMOTE(sampling_strategy={1: 180,0:1000 },random_state=42)
...
def rf_cv(n_estimators, min_samples_split, max_depth, max_features):
    val = cross_val_score(RandomForestClassifier(n_estimators=int(n_estimators),
                          ...
                          random_state = 2),
            data_tr,label_tr,scoring="accuracy",cv=5).mean()
    return val
# 实例化一个bayes优化对象
#贝叶斯优化
...
#模型训练
clf = clf.fit(data_tr,label_tr)
rfc = rfc.fit(data_tr,label_tr)

pre = rfc.predict(data_te)
score_c = clf.score(data_te,label_te)
