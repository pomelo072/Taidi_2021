...
# 读取数据
data = pd.read_csv('../data/建筑业_处理.csv' , engine = "python")
data2 = pd.read_csv('../data/建筑业_处理.csv',engine = "python")
data3 = pd.read_csv('../预测data/建筑业_预测.csv',engine = "python")
data4 = pd.read_csv('../预测data/建筑业_预测.csv',engine = "python")
...
ros = RandomOverSampler(sampling_strategy=0.03,random_state=48)
...
#拆分专家样本集
data_tr, data_te, label_tr, label_te = train_test_split(X_smo,y_smo,test_size=0.3,random_state=31)
print(label_tr.groupby(label_tr).count())
clf1 = xgb.XGBClassifier(n_estimators=525,learning_rate=0.31,
...
clf3 = CatBoostClassifier(loss_function="Logloss",eval_metric="AUC",
                          ...
                          early_stopping_rounds=500,random_state=44)
...
clf6 = svm.SVC(C =2,gamma=2,random_state=44)
...
dataframe = pd.DataFrame({'股票编号':number,'建筑业':zhizaoye})
dataframe.to_csv("建筑业-预测结果.csv", index=False, encoding='GBK')
