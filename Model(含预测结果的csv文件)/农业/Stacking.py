...
data3 = pd.read_csv('../预测data/农、林、牧、渔业_预测.csv',engine = "python")
data4 = pd.read_csv('../预测data/农、林、牧、渔业_预测.csv',engine = "python")
...
smo = SMOTE(sampling_strategy={1:21,0:196 },random_state=43)
...
#拆分专家样本集
data_tr, data_te, label_tr, label_te = train_test_split(X_smo,y_smo,test_size=0.3,random_state=45)
...
clf2 = lgb.LGBMClassifier(boosting_type='gbdt',objective= 'binary',metric =  'binary_logloss,auc',
                          bagging_fraction = 0.98, bagging_freq = 8,
                          feature_fraction = 0.70,lambda_l1 =  0.6,lambda_l2 = 0.7,
                          learning_rate = 0.01,max_bin = 161,max_depth = 9,
                          min_data_in_leaf = 7,min_split_gain = 0.52,
                          n_estimators = 600,num_leaves = 3,
...
clf3 = CatBoostClassifier(loss_function="Logloss",eval_metric="AUC",
                          iterations=2500,learning_rate=0.31,depth=8,verbose=100,bagging_temperature = 0.01,
                          early_stopping_rounds=500)
clf4 = RandomForestClassifier(n_estimators=82,min_samples_split=2,max_features=0.1,max_depth=15)
clf5 = LogisticRegression(C= 0.1, class_weight= 'balanced', max_iter= 10, solver='lbfgs')
clf6 = svm.SVC(C =2,gamma=2)

lr = LogisticRegression()
sclf = StackingCVClassifier(classifiers=[clf1, clf2, ], meta_classifier=lr,random_state=43)
...
dataframe = pd.DataFrame({'股票编号':number,'农、林、牧、渔业':zhizaoye})
dataframe.to_csv("农、林、牧、渔业-预测结果.csv", index=False, encoding='GBK')
